...
Not only is this another intro example, it even has a intro name. Most commands or tools can be configured using arguments, options, flags. Let us take a look at how this can be done:
...
There is no difference with the previous version of `intro_example`. Now, let us pass the argument `-l` to `intro_example2`:
...
```{sh}
viash run src/intro_example2.vsh.yaml -- -a
```

Or even:

```{sh}
viash run src/intro_example2.vsh.yaml -- -a -l
```

For completeness, we also add these arguments to the first intro example:

```{sh}
viash run src/intro_example1.vsh.yaml -- -a -l
```

As you might have expected, since `intro_example1` does not know about any arguments this does not change its behaviour.

## Intro example 3

In this (intro) example, we add an extra argument that corresponds to the path which we want to _list_. The default is the current directory (just like before) but optionally we can provide a different path.
...
```{sh}
viash run src/intro_example3.vsh.yaml -- ./ -a -l
```

```{sh}
viash run src/intro_example3.vsh.yaml -- src/ -a
```
...
The help from the last `intro_example3` does not show a lot of useful information. Let's some documentation regarding the component and its parameters.
...
Suppose `intro_example4` from above is exactly what we need as standalone tool for ourselves or other people to use. Obviously, providing everyone access to [viash] first and then letting them access the `intro_example4.vsh.yaml` file in order to run the above commands would not simplify things at all! However, consider the following:

```{sh}
viash build src/intro_example4.vsh.yaml -o bin
```

This _builds_ an _executable_ (script) `bin/intro_example2` that contains all the functionality that we saw in the above examples. Just to give a few examples:
...
```{sh}
viash build src/intro_example5.vsh.yaml -o bin
```

However, if we list the differences between the previous version of intro example and this one, we note the following:

``` {sh, error=TRUE}
diff bin/intro_example4 bin/intro_example5
```
...
We get a binary `bin/intro_example5` that automatically runs inside Docker:
...
```{sh}
viash build src/intro_example6.vsh.yaml -o bin -p docker2
```

By specifying `-p docker2`, [viash] _knows_ it has to select the corresponding entry from the `platforms` list.

```{sh}
bin/intro_example6 / -l
```

## Wrapping a script

While running a command wrapped as a [viash] component could be useful in _some_ form or another, we will usually want to run something a bit more custom or elaborate. Say you want to run the `intro_example6` component from above but this time filtering out certain files/directories based on their name. We could do just that by means of a simple CLI instruction that we put in a script:
...
```{sh}
cat src/intro_example7.vsh.yaml
```

We get results like this:

```{sh}
viash run src/intro_example7.vsh.yaml -p docker2 -- /etc --filter "^h.*"
```
...
Please note that when we decide to _build_ a `intro_example7` binary (for a specific platform), again this binary is self-contained. It includes the necessary Docker information, command line parsing logic and the script itself. So there is not need for additional customization.
