...
```{sh echo=F, results='asis'}
../scripts/cat_format src/plot_map/script.R
```
...
We can start from the [`rocker/tidyverse:4.0.3`](https://hub.docker.com/r/rocker/tidyverse/) container, which already contains all of the tidyverse packages. 
The extra dependencies can be installed as follows.
...
While most dependencies can be installed directly from CRAN, one package is only available on a GitHub repository.
